    ...
    class MockLLMAdapter:
        def generate_text(self, prompt, model_name, temperature, max_output_tokens):
            ...
            if "Analyze" in prompt:
                return """Insight: User is interested in Python programming.
Fact: User mentioned the project 'Phoenix'.

Insight: AI provided code examples.
Fact: User's name might be Alex (mentioned in passing)."""
            return "[Mock Error]"

    mock_adapter = MockLLMAdapter()

    # Create dummy ParsedConversation data
    meta = ConversationMetadata(conv_id="test_insight_conv", title="Insight Test")
    messages = [
        Message(role="user", content="Tell me about Python decorators for my project 'Phoenix'. My name is Alex."),
        Message(role="assistant", content="Okay, Alex. Decorators in Python are like this: `def decorator...`")
    ] * 50 # Make it long enough to trigger generation
    parsed_conv_test = ParsedConversation(metadata=meta, messages=messages)

    # Test insight generation
    print("\nTesting insight generation...")
    insights = generate_insight_notes(parsed_conv_test, mock_adapter)

    if insights:
        print(f"Generated {len(insights)} insights/facts:")
        for insight in insights:
            print(f"  - {insight.text} (Tokens: {insight.insight_token_count})")
            assert isinstance(insight, InsightNote)
            assert insight.original_conv_id == "test_insight_conv"
        assert len(insights) == 4 # Based on mock response
        print("Insight generation test PASSED")
    else:
        print("Insight generation test FAILED (No insights generated)")

    print("\nInsight Generator testing finished.")
